...
def generate_images(
    ...
    for i, sigma in enumerate(sigmas):
        with torch.no_grad():
            x_denoised = D(x, Tensor([sigma]).to(device))
...
def noising_process(imgs: Tensor, sigmas: Tensor) -> Tensor:
    img_dims = imgs.ndim * [
        None,
    ]
    n_sigma = len(sigmas)
...
def denoiser(
    F: Callable, c_in: Callable, c_out: Callable, c_skip: Callable, c_noise: Callable
) -> Callable:
    def D(x, sigma):
...
def train_ddpm(
    ...
    c_funcs: [Callable, Callable, Callable, Callable],
    N: Callable,
    nb_epochs: int,
    lr: float,
    batch_size: int,
    ...
    D = denoiser(F, c_in, c_out, c_skip, c_noise)
    ...
    dl, info = load_dataset_and_make_dataloaders(
        ...
        num_workers=n_workers,  # you can use more workers if you see the GPU is waiting for the batches
    ...
    for _ in tqdm(range(nb_epochs)):
        ...
        train_losses.append(Tensor(train_loss).mean())
        ...
        val_losses.append(Tensor(val_loss).mean())

    return D, train_losses, val_losses


def plot_imgs(imgs: Tensor, sigmas: Tensor, plot_name: str, plot_title: str):
